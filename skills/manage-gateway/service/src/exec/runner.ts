...
import type { ClientContext } from '../server/auth';
...
function execLocalImpl(
  ...
  return new Promise((resolve, reject) => {
    ...
    // If client has 1Password token, prefer 1Password lookups
    // Fallback: profile creds → profile keychain → global keychain → credentials.json → env
    const missingVars: string[] = [];

    // Use async IIFE to handle 1Password lookups
    (async () => {
      for (const varName of config.authVars) {
        let keyValue: string | undefined;

        // Try 1Password first if client has a token
        if (clientContext?.token) {
          keyValue = await getSharedKey(clientContext.token, varName);
        }

        // Fallback to legacy keychain/file lookup
        if (!keyValue) {
          keyValue = getKey(varName, clientContext?.profile);
        }
      ...
      if (missingVars.length > 0) {
        reject(
          new Error(
            ...
            `Add to 1Password Shared vault or keychain/credentials file`
          )
        );
        return;
      }

      // Continue with process spawn after credentials are resolved
      startProcess();
    })().catch(reject);
