...
See `~/.claude/services/gateway-cc/src/services/github/utils/output.ts` for reference implementation.
...
npx tsx ~/.claude/services/gateway-cc/src/index.ts exec github status --format json
# → { "branch": "andy", "staged": [...], ... }
```
...
```bash
npx tsx ~/.claude/services/gateway-cc/src/index.ts exec github conflicts
# → conflicts[0]{}:
# or
# → conflicts[2]{file,lines}:
#   src/auth.ts,15;23;31
